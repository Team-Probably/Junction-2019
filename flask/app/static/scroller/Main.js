...
class Main {
	...
	loadSpriteSheet() {
		...
		loader.add("wall", "static/scroller/resources/wall.json");
	...
	getAnimatedSprite() {
		...
		for (let i=0; i < 15; i++)
		{
			let texture = PIXI.Texture.from(`static/scroller/png/Walk (${i+1}).png`);
	...
	createBoy()
	{
		let boy = PIXI.Sprite.from('static/scroller/png/Idle (1).png');
